...
function isSameEnvironment(targetPathname) {
    const environmentPath = window.location.pathname === '/search/' ? '/' : window.location.pathname.split('/')[0];
    const isUserInIOTAEnvironment = environmentPath === '/';
    const isEnvironmentSubroute = ['/shimmer/', '/next/'].some(subroute => targetPathname.startsWith(subroute));
    const isUrlFromSameEnvironment = targetPathname.startsWith(environmentPath);

    return (isUserInIOTAEnvironment && !isEnvironmentSubroute) || (!isUserInIOTAEnvironment && isUrlFromSameEnvironment);
}
